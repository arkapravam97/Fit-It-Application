...
start=time.time()
f=pd.read_csv("FMR_Data_R3_14.csv")
X=f["Field"]
Y=f["Combined"]
S=f["Fitted"]
lenX=len(X)
...
coords = np.array(line.intersection(baseline1),dtype=object)
coord1=[]
for i in range(4,len(coords)):
    coord1.append(np.array(coords[i]))
coord2=[]
for i in range(0,len(coord1)):
    coord2.append(coord1[i][0][0])
    coord2.append(coord1[i][1][0])
#coords = coords1[4:3025]
#geoms = [ SG.shape(feat["geometry"]) for feat in coords ]
#coord = [ np.array((geom.xy[0][0], geom.xy[1][0])) for geom in geoms ]
...
for i in range(0,len(coord2)):
    if (coord2[i]<Xmax and coord2[i]>Xmin):
        Xres=float(coord2[i])
print("Xres=", Xres)
...
Xtest=[1*i for i in X]
plt.plot(X, Y, color ="red", label ="data") #plot data
#plt.scatter(Xres, 0, s=10, c='black')
plt.plot(Xtest, final_ans, '-', color ='black', label ="optimized data") #
plt.legend()
plt.savefig("FMR_Data_R3_14.png",dpi=3000)
#plt.show()
